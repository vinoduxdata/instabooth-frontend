...
stateStore.$subscribe((mutation, state) => {
  ...
  if ((state.target == 'ready' || state.target == 'counting') && route.path != '/') {
    // quick fix: receive capture-prep/countdown state but not on indexpage, push router to index
    console.log('capture state received, pushing to index page')
  ...
  if (state.source == 'completed' && state.target == 'present') {
    ...
    if (state.jobmodel.typ === 'image') {
      if (stateStore.imageFinishWithPrint && mediaId) {
        const shareActions = configurationStore.configuration.share.actions
        const printIndex = shareActions.findIndex((a) => a.trigger.ui_trigger.show_button && a.trigger.ui_trigger.title === 'Direct Print')
        const configIndex = printIndex >= 0 ? printIndex : 0
        remoteProcedureCall(`/api/share/actions/${mediaId}/${configIndex}`, 'POST')
      }
    ...
    router.push({ name: 'itempresenter', params: { id: mediaId } })
  }
})
...
const keyUpHandler = (e: KeyboardEvent) => {
  ...
  configurationStore.configuration.actions.image.forEach((action_config, index: number) => {
    if (action_config.trigger.keyboard_trigger.keycode == e.key) {
      stateStore.lastCaptureAction = 'actions/image'
      stateStore.lastCaptureConfigIndex = index
  ...
  configurationStore.configuration.actions.collage.forEach((action_config, index: number) => {
    if (action_config.trigger.keyboard_trigger.keycode == e.key) {
      stateStore.lastCaptureAction = 'actions/collage'
      stateStore.lastCaptureConfigIndex = index
